...
const webpack = (options, callback) => {
	...
	if (Array.isArray(options)) {
		...
		// @zzheng Call "Apply" on all plugins
		// 1. Transfer complier._plugins(real plugins) to hooks life cycle
		// 2. Every plugin will insert attributes into different fields into options
		if (options.plugins && Array.isArray(options.plugins)) {
			for (const plugin of options.plugins) {
				plugin.apply(compiler);
			}
		}
		// @zzheng environment and afterEvnironment are forced to be called here
		// just in case some plugins have these life cycles
	...
	if (callback) {
		...
		if (
			...
			// @zzheng This is for watch dev server, to turn this on is pass callback
			// and watch options to webpack
